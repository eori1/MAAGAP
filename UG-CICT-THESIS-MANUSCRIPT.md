...
4. Develop and evaluate optimization algorithms using linear programming to generate inspector deployment schedules under constraints such as inspector workload capacity, availability, and vehicle access. Success will be measured by demonstrating at least a 15% improvement in allocation efficiency compared to current manual approaches through simulated project scenarios.

5. Evaluate the prototype's software quality using the ISO/IEC 25010 standard, focusing on sub-characteristics such as Functional Suitability, Usability, and Reliability.
...
- **User Interface:** The prototype will be developed as a web-based dashboard using Next.js (React), chosen for its rapid development capabilities and component-based interface design rather than production-grade, enterprise-scale security features.
...
Resource allocation optimization is formulated as a linear programming problem solved using PuLP. The web-based dashboard is developed using Next.js (React), whose built-in API routes serve the REST endpoints for the interactive interface, chosen for its lightweight architecture and rapid development capabilities. The dashboard visualizes risk predictions, resource allocation recommendations, and project portfolio analytics in accessible formats for non-technical decision-makers. Features include risk heatmaps, project detail views, resource allocation visualizations, and automated PDF reports.
