...
## UI revamp Phase 2, Group A + Add new PPA + Import Data (branch `feat/ui-revamp-group-a`)

Group A (Projects, Forecast Engine, Model Validation) chosen first per an `Explore` audit of all 9 remaining pages — see [[02-Decisions-Log]] for the full rationale, the real cross-page duplication found (Badge/StatCard/progress-bar patterns reinvented everywhere, zero pages using tokens.css/Framer Motion), and why `Notice` was deliberately *not* built yet.

- **Projects**: dead "Add new PPA"/"Import Data" buttons and dead sort controls found and initially removed, then real Risk Tier filter added and sort actually wired to `.sort()` logic (previously buttons with no handler at all); result-summary line; `Skeleton`/`EmptyState` states; risk/status badges and progress bars migrated to the shared `Badge`/`ProgressBar` primitives.
- **Forecast Engine**: the fake `85 + Math.random() * 10` "confidence" and a chart synthesized from a single `progress` number were deleted outright. Replaced with **real SHAP feature attributions** already computed by the backend (`predictions.shap_explanation`, `backend/maagap/explainability.py::format_shap_json()`) but never read by the frontend before — new `GET /api/projects/[projectId]` serves it on demand, with real feature names mapped through `frontend/src/lib/shapLabels.ts`.
- **Model Validation**: the 4 stat tiles are now clickable filters (matching Dashboard's triage pattern); migrated to `StatCard`/`Badge`; added a real loading state.
- New shared code: `ProgressBar` primitive, `frontend/src/lib/riskTone.ts` (deduplicated `RISK_TONE` map).

**Add new PPA**: after removing the dead buttons, the user asked for "Add new PPA" to actually be built. Surfaced a real constraint — the ML pipeline is batch/offline, so a manual entry can't get an immediate risk score — resolved with a `risk: "Pending"` / "Pending Assessment" state (`is_manual_entry`/`created_by`/`created_at` columns, `backend/supabase/schema_manual_entry.sql`) shown consistently across Projects/Dashboard/Forecast Engine (the last with a graceful "Not yet assessed" message instead of a 404). Real domain values (project types, 14 agencies, 10 funding sources, 40 municipalities) transcribed from `backend/maagap/config.py` into `frontend/src/lib/ppaOptions.ts`. Follow-up: the user pointed out PPAs need a real name/description, not just an ID — added `project_name`/`description` columns (nullable, existing rows fall back to `project_id`) and required/optional fields in `AddPpaModal`; backfilling real names onto the *existing* cohort is a separate, not-yet-scoped task (see [[06-Current-State-and-Next-Steps]]).

**Import Data**: bulk CSV import, built right after. Preview-before-commit (parse + validate every row client-side, per-row Valid/Error status, only valid rows created on confirm) and a downloadable CSV template, per explicit scoping questions. Shared validation (`frontend/src/lib/ppaValidation.ts::validatePpaRow()`) and project-id allocation (`frontend/src/lib/projectId.ts::createProjectIdAllocator()`, handing out sequential ids per year in-memory so a multi-year batch can't collide with itself) extracted and reused by both the single-add route and the new `POST /api/projects/bulk`. Hand-rolled CSV parser (`frontend/src/lib/csv.ts`) rather than reintroducing `papaparse` (deliberately removed earlier in this project). First UI pass used a raw `<input type="file">`; the user flagged it (via screenshot) as visually inconsistent with the rest of the token-based design system, so it was rebuilt as a drag-and-drop dropzone with a file chip and numbered step labels.
