...
Also loads the Fund Transfer Con sheet (21k+ records, 2013–2026) for
supplementary budget, municipality, funding-source, and liquidation
distributions that are unavailable in the smaller MONITORING REPORT sheet.
"""
...
def _load_ppdo_like_table(filepath):
    """Load a PPDO-like project monitoring table from a workbook.

    This project originally targeted a PPDO 2026 sheet with a fixed layout.
    In the thesis workbook variant, the closest equivalent is the
    `MONITORING REPORT Con` sheet (project-level monitoring records).
    """
    # Preferred sheet in the provided thesis workbook
    try:
        df = pd.read_excel(filepath, sheet_name="MONITORING REPORT Con")
        colmap = _normalise_columns(df)
        df = df.rename(columns=colmap)
        return df, "MONITORING REPORT Con"
    except Exception:
        pass

    # Fallback: previous fixed-layout sheet ("Sheet1", headerless; rows start at 8)
...
def load_and_clean_ppdo(filepath=None):
    ...
    data, sheet_used = _load_ppdo_like_table(filepath)

    # Map the thesis workbook columns into the canonical schema expected by
    # downstream feature engineering + distribution extraction.
    if sheet_used == "MONITORING REPORT Con":
        # Drop the unnamed helper/index column (often all NaN or redundant numbering)
        junk_cols = [c for c in data.columns if c.strip() == "" or c.startswith("unnamed")]
        if junk_cols:
        ...
        # Merge status + remarks into a single remarks text field
        ...
        # Brief description: use file/source label when available (helps project_type keywording a bit)
...
# ---------------------------------------------------------------------------
# Fund Transfer Con — supplementary real data (21k rows, 2013–2026)
# ---------------------------------------------------------------------------
...
def _normalise_fund_source(raw: str) -> str:
    """Map raw Fund Transfer source labels → canonical FUNDING_SOURCES entries."""
    upper = str(raw).upper().strip()
    for key, canon in _FUND_SOURCE_MAP.items():
        if key in upper:
            return canon
    return "General Fund"


def load_fund_transfer_con(filepath=None):
    """Load and clean the 'Fund Transfer Con' sheet.

    Returns a cleaned DataFrame or None if the sheet is unavailable.
    """
    filepath = filepath or REAL_DATA_FILE
    try:
        df = pd.read_excel(filepath, sheet_name="Fund Transfer Con", header=0)
    except Exception:
        return None

    df.columns = [str(c).strip() for c in df.columns]
    # Drop unnamed trailing helper columns
    df = df[[c for c in df.columns if not c.startswith("Unnamed")]].copy()

    # Deduplicate column names (e.g. multiple 'Remarks' columns in Fund Transfer Con)
    seen = {}
    new_cols = []
    ...
    # Keep only rows with a positive budget
    df = df[df["approved_budget"].notna() & (df["approved_budget"] > 0)].copy()

    # Derived fields
    df["is_liquidated"] = (
        df["status"].astype(str).str.lower().str.contains("liquidated", na=False)
    )
    df["is_monitored"] = (
        df["status"].astype(str).str.lower().str.contains("monitored", na=False)
    )
    df["project_type"] = df.apply(_classify_type, axis=1)
    df["source_of_fund_canon"] = df["source_of_fund"].astype(str).apply(_normalise_fund_source)

    return df


def extract_fund_transfer_distributions(df_ft):
    """Derive usable distributions from the Fund Transfer Con sheet.

    Returns a dict that enriches (or replaces) distributions from the
    smaller MONITORING REPORT sheet.
    """
    ...
    # Budget distribution — larger, more representative sample
    ...
    if "municipality" in df_ft.columns:
        muni_vc = df_ft["municipality"].dropna().value_counts(normalize=True)
        dist["municipality_probs"] = muni_vc.to_dict()

    # Funding source distribution (canonical)
    if "source_of_fund_canon" in df_ft.columns:
        fs_vc = df_ft["source_of_fund_canon"].value_counts(normalize=True)
        dist["funding_source_probs"] = fs_vc.to_dict()

    # Project type distribution from real project names
    if "project_type" in df_ft.columns:
        type_vc = df_ft["project_type"].value_counts(normalize=True)
        dist["ft_type_probs"] = type_vc.to_dict()

    # Liquidation rate: proxy for completion / no-delay
    if "is_liquidated" in df_ft.columns:
        dist["liquidation_rate"] = float(df_ft["is_liquidated"].mean())

    # Overrun signal: projects with balance > 0 = partially unliquidated
    ...
    if "year" in df_ft.columns:
        yr_vc = df_ft["year"].dropna().value_counts(normalize=True)
        dist["fund_transfer_year_probs"] = yr_vc.to_dict()
