...
class FeatureEngineer:
    """Encapsulates stateful feature transformations, scalers, and label encoders."""
    ...
    def __init__(self):
        ...
        self.temporal_scaler: Optional[MinMaxScaler] = None
        self.static_scaler: Optional[MinMaxScaler] = None # Currently unused in original but kept for signature
        self.static_feature_names: List[str] = []
        self.temporal_feature_names: List[str] = self.TEMPORAL_FEATURES.copy()
    ...
    def build_static_features(self, df_projects: pd.DataFrame) -> Tuple[np.ndarray, List[str], Optional[MinMaxScaler], Dict[str, LabelEncoder]]:
        """Label-encode categoricals + engineered interactions -> feature matrix."""
        logger.info("Building static features...")
        df = self._add_engineered_features(df_projects)
        ...
        for col in self.STATIC_CATEGORICAL:
            le = LabelEncoder()
        ...
        X_df = pd.concat([
            df[self.STATIC_NUMERIC].fillna(0).astype(float),
            df[engineered_cols].fillna(0).astype(float),
            cat_encoded.astype(float),
        ], axis=1)
        
        self.static_feature_names = list(X_df.columns)
        X = X_df.values.astype(np.float32)
        
        logger.info(f"Generated static feature matrix: {X.shape}")
        return X, self.static_feature_names, self.static_scaler, self.label_encoders

    def build_temporal_sequences(self, df_projects: pd.DataFrame, df_quarterly: pd.DataFrame) -> Tuple[np.ndarray, List[str], MinMaxScaler]:
        """Pad/truncate quarterly records into fixed-length 3-D tensor for LSTM."""
        logger.info("Building temporal sequences...")
        n_features = len(self.TEMPORAL_FEATURES)
        ...
        all_temporal = df_quarterly[self.TEMPORAL_FEATURES].values.astype(np.float32)
        self.temporal_scaler.fit(all_temporal)

        for i, pid in enumerate(project_ids):
            if pid not in q_grouped.groups:
                continue
            grp = q_grouped.get_group(pid).sort_values("quarter")
            vals = grp[self.TEMPORAL_FEATURES].values.astype(np.float32)
            vals_scaled = self.temporal_scaler.transform(vals)
            t = min(len(vals_scaled), LSTM_MAX_TIMESTEPS)
            X[i, :t, :] = vals_scaled[:t]

        logger.info(f"Generated temporal tensor: {X.shape}")
        return X, self.temporal_feature_names, self.temporal_scaler

    @staticmethod
    def build_targets(df_projects: pd.DataFrame) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
...
def split_data(n: int, train_ratio: float = 0.70, val_ratio: float = 0.15, seed: int = 42) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
...
def build_static_features(df_projects: pd.DataFrame):
    return FeatureEngineer().build_static_features(df_projects)

def build_temporal_sequences(df_projects: pd.DataFrame, df_quarterly: pd.DataFrame):
    return FeatureEngineer().build_temporal_sequences(df_projects, df_quarterly)

def build_targets(df_projects: pd.DataFrame):
    return FeatureEngineer.build_targets(df_projects)
