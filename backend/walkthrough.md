...
├── MAAGAP_Objective1_Clean.ipynb  # Interactive notebook (same pipeline)
├── maagap/
...
### `DataPreprocessor.load_and_clean_ppdo()`
...
### `DataPreprocessor.extract_distributions(df)`
...
### `DataPreprocessor.load_fund_transfer_con()` *(NEW)*
...
### `DataPreprocessor.extract_fund_transfer_distributions(df_ft)` *(NEW)*
Returns:
...
### The Delay Probability Formula (`SyntheticDataGenerator._compute_delay_risk`)
...
### The Overrun Probability Formula (`SyntheticDataGenerator._compute_overrun_risk`)
...
### Risk Category Assignment (`SyntheticDataGenerator._risk_category`)
...
### Stage 1a: Random Forest (`TreeModelTrainer.train_random_forest`)
...
### Stage 1b: XGBoost (`TreeModelTrainer.train_xgboost`)
...
### Stage 2: LSTM (`LSTMTrainer.train_lstm`)
...
This is computed by `MetaEnsembleTrainer.meta_ensemble_percent_contributions()`.
...
### `get_risk_tier(scores)` — Tier Assignment
...
### `check_logic_consistency(scores, tiers)`
...
### `ResourceOptimizer.run_monte_carlo_baseline()`
...
### `ResourceOptimizer.evaluate_allocation_efficiency(selected_idx, risk_scores, risk_tiers)`
...
### `ResourceOptimizer.analyze_improvement()`
